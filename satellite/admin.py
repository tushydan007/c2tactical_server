...
class SatelliteImageAdmin(GISModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "description", "acquisition_date")}),
        (
            "Image Files",
            {
                "fields": (
        ...
        (
            "Geospatial Data",
            {"fields": ("bounds", "center_point"), "classes": ("collapse",)},
        ),
        (
            "Metadata",
            {
    ...
    def trigger_optimization(self, request, queryset):
        ...
        self.message_user(request, f"Optimization triggered for {count} image(s).")
    ...
    def run_threat_detection_analysis(self, request, queryset):
        ...
        self.message_user(
            request, f"Threat detection analysis initiated for {count} image(s)."
        )
    ...
    def run_change_detection_analysis(self, request, queryset):
        ...
        self.message_user(
            request, f"Change detection analysis initiated for {count} image(s)."
        )
...
class AnalysisResultAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            ...
            {"fields": ("satellite_image", "analysis_type", "status", "initiated_by")},
        ),
        (
            "Timestamps",
...
class ThreatDetectionAdmin(GISModelAdmin):
    ...
    list_filter = ["severity", "threat_type", "verified", "acknowledged", "detected_at"]
    ...
    fieldsets = (
        ...
        ("Location", {"fields": ("location", "pixel_coordinates", "area")}),
        ("Description", {"fields": ("description", "technical_details")}),
        (
            "Verification",
            {
                "fields": (
    ...
    def confidence_display(self, obj):
        ...
        color = (
            "#22c55e"
            if percentage >= 80
            else "#f59e0b" if percentage >= 60 else "#ef4444"
        )
        return format_html(
