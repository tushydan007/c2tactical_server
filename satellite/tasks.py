...
def optimize_satellite_image(self, image_id: int):
    ...
    try:
        ...
        logger.info(f"Starting optimization for image {image_id}: {satellite_image.name}")
...
def run_satellite_analysis(self, analysis_id: int):
    ...
    try:
        analysis = AnalysisResult.objects.select_related('satellite_image').get(id=analysis_id)
        logger.info(
