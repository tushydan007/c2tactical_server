...
class ThreatDetector:
    ...
    def _pixel_to_geo(self, x: float, y: float) -> Tuple[float, float]:
        ...
        try:
            ...
            logger.error(f"Error converting pixel ({x}, {y}) to geo coordinates: {str(e)}")
    ...
    def _validate_coordinates(self, lon: float, lat: float) -> bool:
        ...
        try:
            ...
            if not (min_lon - tolerance * lon_range <= lon <= max_lon + tolerance * lon_range):
                logger.warning(f"Longitude {lon} outside image bounds [{min_lon}, {max_lon}]")
                return False
            
            if not (min_lat - tolerance * lat_range <= lat <= max_lat + tolerance * lat_range):
                logger.warning(f"Latitude {lat} outside image bounds [{min_lat}, {max_lat}]")
                return False
                        ...
                        for region in regions:
                            if region.area > 100:
                                ...
                                if not self._validate_coordinates(lon, lat):
                                    logger.warning(f"Skipping fire detection with invalid coordinates: ({lon}, {lat})")
                                    continue
                                ...
                                logger.debug(f"Fire detected at pixel ({global_x}, {global_y}) -> geo ({lon}, {lat})")
    ...
    def detect_structural_damage(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            height, width = self.dataset.height, self.dataset.width
            logger.info(f"Processing image of size {width}x{height} for structural damage")
                    ...
                    try:
                        ...
                        if edge_density > 0.2:  # Much higher threshold
                            ...
                            if cy is not None and cx is not None:
                                ...
                                # Validate coordinates before adding detection
                                if not self._validate_coordinates(lon, lat):
                                    logger.warning(f"Skipping damage detection with invalid coordinates: ({lon}, {lat})")
                                    continue
                                ...
                                logger.debug(f"Damage detected at pixel ({global_x}, {global_y}) -> geo ({lon}, {lat})")
    ...
    def detect_vehicle_concentrations(self) -> List[Dict[str, Any]]:
        ...
        try:
            height, width = self.dataset.height, self.dataset.width
            logger.info(f"Processing image of size {width}x{height} for vehicle detection")
            all_keypoints = []
                ...
                try:
                    ...
                    for cluster_id in unique_clusters:
                        ...
                        if len(cluster_points) >= 5:
                            ...
                            # Validate coordinates before adding detection
                            if not self._validate_coordinates(lon, lat):
                                logger.warning(f"Skipping vehicle detection with invalid coordinates: ({lon}, {lat})")
                                continue
                            ...
                            logger.debug(f"Vehicles detected at pixel ({centroid_x}, {centroid_y}) -> geo ({lon}, {lat})")
