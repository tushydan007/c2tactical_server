...
class AnalysisProcessor:
        ...
        try:
            ...
            with ThreatDetector(self.image_path) as detector:
                ...
                if self.analysis.analysis_type == 'threat_detection':
                    ...
                    self._log('warning', f'Unknown analysis type: {self.analysis.analysis_type}')
            ...
            self._log('info', f'Analysis completed successfully. Found {len(all_detections)} threats.')
            ...
            self.analysis.completed_at = timezone.now()
            self.analysis.save(update_fields=['status', 'error_message', 'completed_at'])
    ...
    def _generate_summary(self, detections: List[Dict[str, Any]]) -> str:
        ...
        for threat_type, count in sorted(type_counts.items(), key=lambda x: x[1], reverse=True):
            desc = threat_descriptions.get(threat_type, threat_type.replace('_', ' ').title())
